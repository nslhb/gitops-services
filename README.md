...
A [tenant repository](https://github.com/fluxcd/flux2-multi-tenancy/tree/devops) contains the following top directories:
...
Assuming a platform admin wants to create a tenant named `devops` with access to the `apps` namespace.

Create the tenant base directory:

```sh
mkdir -p ./tenants/base/devops
```

Generate the namespace, service account and role binding for the devops:

```sh
flux create tenant devops --with-namespace=apps \
    --export > ./tenants/base/devops/rbac.yaml
```
...
flux create source git devops \
    ...
    --branch=main \
    --export > ./tenants/base/devops/sync.yaml

flux create kustomization devops \
    --namespace=apps \
    --service-account=devops \
    --source=GitRepository/devops \
    --path="./" \
    --export >> ./tenants/base/devops/sync.yaml
```
...
```sh
cd ./tenants/base/devops/ && kustomize create --autodetect
```
...
devops's repository, and it will reconcile the `./staging` directory from the tenant's repo
using the `devops` service account. Since that service account is restricted to the `apps` namespace,
the devops repository must contain Kubernetes objects scoped to the `apps` namespace only.
...
Then we setup [cluster policies](policy/kyverno-policies/flux-multi-tenancy.yaml) 
...
NAME    	READY	MESSAGE 
devops	True 	Fetched revision: devops/ca8ec25405cc03f2f374d2f35f9299d84ced01e4
```
...
```sh
flux -n apps create secret git devops-auth \
    --url=ssh://git@github.com/<org>/<devops> \
    --export > ./tenants/base/devops/auth.yaml
```

Print the SSH public key and add it as a read-only deploy key to the devops repository:
...
The GitHub token must have read-only access to the devops repository.

### Configure Git authentication

Encrypt the `devops-auth` secret's data field with sops:

```sh
sops --encrypt \
    --pgp=1F3D1CED2F865F5E59CA564553241F147E7C5FA4 \
    --encrypted-regex '^(data|stringData)$' \
    --in-place ./tenants/base/devops/auth.yaml
```
